...
class InvoicePaymentConfiguration {
  const InvoicePaymentConfiguration({required this.paymentDocTypeId, required this.allocationDocTypeId, required this.currencyId});
...
int? selectDefaultSalesRepId(List<Map<String, dynamic>> salesReps, int? userId) {
...
List<Map<String, dynamic>> buildFifoAllocationLines({
  ...
  final paymentTotal = paymentQueue.fold<int>(0, (sum, item) => sum + (item['remaining'] as int));
  final invoiceTotal = invoiceQueue.fold<int>(0, (sum, item) => sum + (item['remaining'] as int));
  if (paymentTotal != invoiceTotal) {
    throw ArgumentError('Los métodos de pago deben coincidir con el monto asignado a facturas.');
  }

  final lines = <Map<String, dynamic>>[];
  var paymentIndex = 0;
  var invoiceIndex = 0;
  while (paymentIndex < paymentQueue.length && invoiceIndex < invoiceQueue.length) {
    ...
    final allocatedCents = paymentRemaining < invoiceRemaining ? paymentRemaining : invoiceRemaining;
...
List<Map<String, dynamic>> buildInvoicePaymentBatch({
  ...
  final lines = buildFifoAllocationLines(payments: payments, invoices: invoices, organizationId: organizationId, bPartnerId: bPartnerId);
...
Future<InvoicePaymentConfiguration> resolveInvoicePaymentConfiguration({required BuildContext context, required int bankAccountId}) async {
  await usuarioAuth(context: context);
  final cacheKey = '${Token.auth.hashCode}:${Token.client}:${Token.organitation}:$bankAccountId';
  if (_invoicePaymentConfigurationCacheKey == cacheKey && _invoicePaymentConfigurationCache != null) {
    return _invoicePaymentConfigurationCache!;
  }
  final paymentDocTypeId = await _fetchUniqueDocTypeId("DocBaseType eq 'ARR' and IsSOTrx eq true");
  final allocationDocTypeId = await _fetchUniqueDocTypeId("DocBaseType eq 'CMA'");
  final bankResponse = await get(Uri.parse('${EndPoints.cBankAccount}/$bankAccountId'), headers: _headers());
  if (bankResponse.statusCode != 200) {
    throw Exception('No se pudo consultar la moneda de la cuenta bancaria (${bankResponse.statusCode}).');
  }
...
Future<Map<String, dynamic>> postInvoicePaymentBatch({
  ...
  try {
    await usuarioAuth(context: context);
    final configuration = await resolveInvoicePaymentConfiguration(context: context, bankAccountId: bankAccountId);
    _validateInvoiceContext(invoices: invoices, organizationId: organizationId, currencyId: configuration.currencyId);
    ...
    final response = await post(Uri.parse(EndPoints.batch), headers: _headers(), body: jsonEncode(payload));
    final dynamic decoded = response.bodyBytes.isEmpty ? null : json.decode(utf8.decode(response.bodyBytes));
    final validationError = validateInvoicePaymentBatchResponse(decoded, paymentCount: payments.length);
    if (response.statusCode == 200 && validationError == null) {
      return {'success': true, 'response': decoded};
    }
    final message = validationError ?? 'Error en el batch (${response.statusCode}): ${utf8.decode(response.bodyBytes)}';
    CurrentLogMessage.add(message, level: 'ERROR', tag: 'postInvoicePaymentBatch');
    return {'success': false, 'message': message};
  } catch (error) {
    CurrentLogMessage.add('Excepción en pago y asignación: $error', level: 'ERROR', tag: 'postInvoicePaymentBatch');
    return {'success': false, 'message': error.toString()};
  }
}

String? validateInvoicePaymentBatchResponse(dynamic decoded, {required int paymentCount}) {
...
Future<int> _fetchUniqueDocTypeId(String filter) async {
  final response = await get(Uri.parse('${EndPoints.cDocType}?\$filter=$filter&\$select=Name,DocBaseType,IsSOTrx'), headers: _headers());
  if (response.statusCode != 200) {
    throw Exception('No se pudo resolver el tipo de documento (${response.statusCode}).');
  }
  final decoded = json.decode(utf8.decode(response.bodyBytes));
  final records = (decoded['records'] as List?) ?? const [];
  if (records.length != 1) {
    throw Exception('Se esperaba un único tipo de documento para "$filter" y se encontraron ${records.length}.');
  }
...
void _validateInvoiceContext({required List<Map<String, dynamic>> invoices, required int organizationId, required int currencyId}) {
  if (invoices.isEmpty) {
  ...
  for (final invoice in invoices) {
    ...
    if (invoiceOrgId == null || invoiceOrgId != organizationId) {
      throw ArgumentError('Todas las facturas deben pertenecer a la organización activa.');
    }
    if (invoiceCurrencyId == null || invoiceCurrencyId != currencyId) {
      throw ArgumentError('Todas las facturas deben usar la moneda de la cuenta bancaria del POS.');
    }
  }
}
