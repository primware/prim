  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      POSPrinter.headerPhone =
          record['Phone2']?.toString() ?? record['Phone']?.toString();
...
Future<void> _loadDynamicPOSPrinterConfig() async {
  ...
  try {
    final response = await get(
      Uri.parse(
        '${EndPoints.cdsPOSPrinterConfig}?\$filter=C_POS_ID eq ${POS.cPosID}',
      ),
      headers: {
        'Content-Type': 'application/json; charset=UTF-8',
        'Authorization': Token.auth!,
...
Future<void> _loadDiscountTaxConfig() async {
  ...
  try {
    final chargeResponse = await get(
      Uri.parse(
        '${EndPoints.cCharge}?\$filter=C_Charge_ID eq $chargeId&\$select=C_TaxCategory_ID',
      ),
      headers: {
        'Content-Type': 'application/json; charset=UTF-8',
        'Authorization': Token.auth!,
    ...
    if (chargeResponse.statusCode != 200) {
      throw Exception(
        'No se pudo consultar C_Charge: ${chargeResponse.statusCode} ${chargeResponse.body}',
      );
    }

    final chargeRecords =
        (json.decode(utf8.decode(chargeResponse.bodyBytes))['records']
            as List?) ??
        const [];
    final dynamic taxCategoryField = chargeRecords.isNotEmpty
        ? chargeRecords.first['C_TaxCategory_ID']
        : null;
    final dynamic taxCategoryId = taxCategoryField is Map
        ? taxCategoryField['id']
        : null;
    if (taxCategoryId is! num) {
      throw Exception(
        'El charge $chargeId no tiene C_TaxCategory_ID configurado.',
      );
    }
    ...
    if (categoryResponse.statusCode != 200) {
      throw Exception(
        'No se pudo consultar C_TaxCategory: ${categoryResponse.statusCode} ${categoryResponse.body}',
      );
    }

    final categoryRecords =
        (json.decode(utf8.decode(categoryResponse.bodyBytes))['records']
            as List?) ??
        const [];
    final List<dynamic> taxes = categoryRecords.isNotEmpty
        ? (categoryRecords.first['C_Tax'] as List? ?? const [])
        : const [];
    if (taxes.isEmpty) {
      throw Exception(
        'La categoría ${taxCategoryId.toInt()} no tiene impuestos configurados.',
      );
    }
    ...
    if (taxId is! num || rate == null) {
      throw Exception(
        'El primer impuesto de la categoría ${taxCategoryId.toInt()} no tiene ID o Rate válido.',
      );
    }
    ...
    CurrentLogMessage.add(
      'Configuración fiscal del descuento no disponible: $e',
      level: 'ERROR',
      tag: '_loadDiscountTaxConfig',
    );
  }
}
...
Future<void> _loadPOSData(BuildContext context) async {
  try {
    ...
    if (response.statusCode == 200) {
      ...
      final dynamic rawBankAccountId = bankAccount is Map
          ? bankAccount['id']
          : bankAccount;
      ...
      POS.discountChargeID =
          discountChargeId is num && discountChargeId.toInt() > 0
          ? discountChargeId.toInt()
          : null;
