...
Future<void> _loadPOSData(BuildContext context) async {
  try {
    ...
    final response = await get(
      Uri.parse('${EndPoints.cPos}?\$filter=$filter&\$expand=C_DocType_ID,C_DocTypeRefund_ID,C_BankAccount_ID'),
      headers: {'Content-Type': 'application/json; charset=UTF-8', 'Authorization': Token.auth!},
    );
    ...
    if (response.statusCode == 200) {
      ...
      POS.bankAccountID = rawBankAccountId is num ? rawBankAccountId.toInt() : int.tryParse(rawBankAccountId?.toString() ?? '');
